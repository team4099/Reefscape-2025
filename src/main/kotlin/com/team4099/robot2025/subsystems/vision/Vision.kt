...
class Vision(vararg cameras: CameraIO) : SubsystemBase() {
    ...
    var lastTrigVisionUpdate = TimestampedTrigVisionUpdate(Clock.fpgaTime, -1, Transform2d(Translation2d(), 0.degrees))
    ...
    override fun periodic() {
        ...
        for (i in io.indices) { closestReefTags[i] = null }
        ...
        for (instance in io.indices) {
                            ...
                            Transform2d(
                                PhotonUtils.estimateCameraToTarget(
                                    PhotonUtils.estimateCameraToTargetTranslation(
                                        cameraDistanceToTarget.inMeters, Rotation2d(-tag.yaw.degrees.inRadians)
                                    ),
                                ...
                                .transformBy(
                                    Transform3d(
                                        Translation3d(
                                            ...
                                            VisionConstants.REEF_TAG_HEIGHT -  cameraPoses[instance].translation.z
                                        ),
            ...
            closestReefTag = reefTags.minByOrNull { it.second.x + it.second.y}

            closestReefTags[instance] = closestReefTag

            Logger.recordOutput("Vision/${VisionConstants.CAMERA_NAMES[instance]}/cornerDetections}", cornerData.toDoubleArray())

            Logger.recordOutput("Vision/${VisionConstants.CAMERA_NAMES[instance]}/closestReefTagID}", closestReefTag?.first ?: -1)

            Logger.recordOutput("Vision/${VisionConstants.CAMERA_NAMES[instance]}/closestReefTagPose}", closestReefTag?.second?.transform3d)


        }


        closestReefTagAcrossCams = closestReefTags.minByOrNull { it.value?.second?.translation?.norm ?: 1000000.meters}


        Logger.recordOutput(
            "Vision/ClosestReefTagAcrossAllCams/CameraID", if (closestReefTagAcrossCams != null) VisionConstants.CAMERA_NAMES[closestReefTagAcrossCams?.key ?: -1]  else "None"
        )
        ...
        Logger.recordOutput(
            "Vision/ClosestReefTagAcrossAllCams/ReefTagPose", closestReefTagAcrossCams?.value?.second?.transform3d
        )

        if (closestReefTagAcrossCams?.key != null && closestReefTagAcrossCams?.value != null) {

            lastTrigVisionUpdate = TimestampedTrigVisionUpdate(
                inputs[closestReefTagAcrossCams?.key ?: 0].timestamp,
                closestReefTagAcrossCams?.value?.first ?: -1,
                Transform2d(
                    Translation2d(closestReefTagAcrossCams?.value?.second?.translation?.x ?: 0.meters,
