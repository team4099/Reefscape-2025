    ...
    */
    ...
    when (currentState) {
      ...
      SuperstructureStates.MANUAL_RESET -> {
        arm.currentRequest =
          Request.ArmRequest.OpenLoop(ArmTunableValues.ArmVoltages.retractVoltage.get())
        elevator.currentRequest =
          Request.ElevatorRequest.OpenLoop(ElevatorTunableValues.slamVoltage.get())
      }
      ...
      SuperstructureStates.PREP_INTAKE_ALGAE -> {
        if (algaeIntakeLevel != lastAlgaeIntakeLevel) {
          ...
          if (elevator.isAtTargetedPosition) {
            arm.currentRequest =
              Request.ArmRequest.OpenLoop(ArmTunableValues.ArmVoltages.extendVoltage.get())
          }
        }
