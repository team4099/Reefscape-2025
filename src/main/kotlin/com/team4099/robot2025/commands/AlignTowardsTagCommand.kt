...
class AlignTowardsTagCommand(
  ...
  override fun execute() {
    ...
    if (cams.size >= 2 && cams[0].cameraTargets.size > 0 && cams[1].cameraTargets.size > 0) {
      ...
      vision.isAligned =
        Math.abs(raven1.yaw - raven2.yaw) < 2 && raven1.fiducialId == raven2.fiducialId
    }
    ...
    //        val angleOffset = (180.degrees - lastUpdate.robotTReefTag.rotation.absoluteValue) -
    // lastUpdate.robotTReefTag.rotation
    //        val thetaFeedback =
    // thetaPID.calculate(lastUpdate.robotTReefTag.rotation.absoluteValue, 1.0.radians)
    //
    //        if (!vision.isAligned && thetaPID.error.absoluteValue > 5.degrees &&
